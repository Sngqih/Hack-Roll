...
class TalkingObjectsApp {
    constructor() {
        ...
        this.localLLM = null; // Local Transformers.js model instance (disabled)
        this.localLLMLoading = false; // Track if local LLM is loading
        this.ollamaUrl = null; // Disable Ollama entirely
        this.ollamaModel = null; // Disable Ollama model selection
        this.llmMode = 'none'; // Disable automatic/model-driven LLM use
    ...
    async init() {
        ...
        // Initialize IntelliDisplay - LLM disabled, rely on canned chaos
        this.updateIntelliDisplay(true, 'LLM disabled - using canned chaos');
    ...
    async generateLLMResponse(obj, context, userMessage = null) {
        // Hardcoded unhinged responses only; no external/local LLMs are used
        this.llmEnabled = true; // keep true so dialogue callers still route here
        this.llmInUse = false;
        this.updateIntelliDisplay(true, 'LLM disabled - using canned chaos');
        return this.getUnhingedResponse(obj, context, userMessage);
    }
